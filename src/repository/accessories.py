...
from src.schemas import AccessoriesSchemaCRUD, AccessoriesSchemaORM
...
class AccessoriesRepository:
    ...
    async def get_all(self) -> Sequence[AccessoriesSchemaORM]:
        ...
        return paginate([AccessoriesSchemaORM.model_validate(accessories_model) for accessories_model in accessories_models])
    ...
    async def create(self, new_accessories: AccessoriesSchemaCRUD) -> AccessoriesSchemaORM:
    ...
    async def update(self, _id: UUID4, update_accessories: AccessoriesSchemaCRUD) -> None:
        stmt = update(Accessories).where(Accessories.id == _id).values(**update_accessories.model_dump())
